...
for i=1:Nx-1
    i
    A=ones(Ny-3,1).*(-v(3:end-1,i)/(4*dy)-nu/(2*dy^2));
    C=ones(Ny-3,1).*(v(2:end-2,i)/(4*dy)-nu/(2*dy^2));
    B=u(2:end-1,i)/(dx)+nu/(dy^2); 
    ...
    for j=2:Ny-1
        D(j-1)=u(j,i)*(u(j,i)/(dx)-nu/dy^2)...
              +u(j+1,i)*(nu/(2*dy^2)-v(j,i)/(4*dy))...
              +u(j-1,i)*(nu/(2*dy^2)+v(j,i)/(4*dy));
    end

    D(end)=D(end)-u(end,1)*(-nu/(2*dy^2)+v(j,i)/(4*dy));
    
    D(1)=D(1)-u(1,i)*(-nu/(2*dy^2)-v(2,i)/(4*dy));
...
%PLOTTING STUFF
figure;
plot(x, delta_blausius, 'r-', 'LineWidth', 1.5); hold on;
...
figure
plot(x, delta_star_blausius, 'r-', 'LineWidth', 1.5); hold on;
